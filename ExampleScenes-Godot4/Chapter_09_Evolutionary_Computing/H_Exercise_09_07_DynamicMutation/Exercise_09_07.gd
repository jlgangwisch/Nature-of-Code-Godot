#Exercise 9_7
#Explore the idea of a dynamic mutation rate. 
#For example, try calculating a mutation rate that inversely correlates with the 
#average fitness of the parent phrases so that higher fitness results in fewer 
#mutations. Does this change affect the behavior of the overall system and how 
#quickly the target phrase is found?
extends Node2D

var population_num = 200
var population = []
...
func initialize_population():
	for i in range(population_num):
		var d = exercise_09_07_DNA.new(target)
		population.append(d)
...
func reproduce(mating_pool):
	...
	var child = exercise_09_07_DNA.new(parent_a.target_string)
	parent_a.crossover(parent_b, child)
	var low_mutation : bool
	#Exercise 9_7
	if parent_a.fitness > 0.1 and parent_b.fitness > 0.1:
		low_mutation = true
	else:
		low_mutation = false
	child.mutate(low_mutation)
