...
#according to the number of correct characters. Note that you’ll likely have to 
#normalize the fitness values to a range from 0 to 1 so they can be added to the 
#mating pool a reasonable number of times, or use a different weighted-selection method.
...
func initialize_population():
	for i in range(population_num):
		var d = exercise_09_08_DNA.new(target)
		population.append(d)
...
func reproduce(mating_pool):
	...
	var child = exercise_09_08_DNA.new(parent_a.target_string)
