...
func _ready():
	extent = randf_range(25, 200)
	var shape = CircleShape2D.new()
	shape.set_radius(extent)
	collider.set_shape(shape)
	
	
	position = Vector2(randf_range(0+extent, viewport_size.x-extent), randf_range(0+extent, viewport_size.y-extent) )
	print(position)
...
func _gonnaha():
	if extent > 100:
		var particle_system = PARTICLE_SYSTEM.instantiate()
...
func _draw():
	...
	var size = Vector2(2*extent,2*extent)
