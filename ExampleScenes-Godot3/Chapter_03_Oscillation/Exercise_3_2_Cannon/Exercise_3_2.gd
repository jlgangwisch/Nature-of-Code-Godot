...
func _ready():
	...
	for mover in movers:
		...
		mover.mass = randf_range(10,100)
		mover.get_node("ColorRect").size = Vector2(mover.mass,mover.mass)
		mover.get_node("ColorRect").position = Vector2(-mover.mass/2, -mover.mass/2)
		mover.velocity = Vector2(randf_range(-1,1), randf_range(-1,1))
...
func _physics_process(delta):
	...
	if Input.is_action_just_released("ui_up"):
		...
		mover.mass = randf_range(10,100)
		mover.get_node("ColorRect").size = Vector2(mover.mass/2,mover.mass)
		mover.get_node("ColorRect").position = Vector2(-mover.mass/2, -mover.mass/2)
