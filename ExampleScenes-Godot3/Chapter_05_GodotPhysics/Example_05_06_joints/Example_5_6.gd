...
@export var loops: int = 20
@export var softness = 0.1 # (float, 0, 1, 0.1)
@export var bias = 0.9 # (float, 0, 1, 0.1)
...
@export var damping = 0.1 # (float, 0, 1, 0.1)
...
func _process(delta):
	if Input.is_action_just_released("left_mouse"):
		...
		bouncer.rotation = randf_range(0, 2*PI)
