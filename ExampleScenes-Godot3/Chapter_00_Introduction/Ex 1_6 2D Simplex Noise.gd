...
func draw_noise_to_image(function_image):
	false # function_image.lock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	var period_x = 0
	for x in viewport_size.x:
		...
		for y in viewport_size.y:
			...
			period_y += 0.1
		period_x += 0.1
	false # function_image.unlock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
